...
describe('Screenshot Testing: Check if screenshots are correct', function() {
  ...
  before(async function() {
    ...
    devices.forEach(device => {
      if (!fs.existsSync(`${testDir}/${device.name}`))
        fs.mkdirSync(`${testDir}/${device.name}`)
    })
  })
...
function _compareScreenshots(fileName) {
  return new Promise(resolve => {
    const img1 = fs
      ...
      .pipe(new PNG())
      .on('parsed', _doneReading)
    const img2 = fs
      .createReadStream(`${groundtruthDir}/${fileName}.png`)
      .pipe(new PNG())
      .on('parsed', _doneReading)
    ...
    function _doneReading() {
      ...
      if (numDiffPixels > 0) {
        diff
          .pack()
          .pipe(fs.createWriteStream(`${testDir}/${fileName}-diff.png`))
      }
