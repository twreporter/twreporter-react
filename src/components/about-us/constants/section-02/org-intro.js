export const foundationIntro = {
  chinese:
    '「財團法人報導者文化基金會」由童子賢先生個人捐助 500 萬元成立，創辦人何榮幸擔任執行長，後續接受各界贊助維持營運。',
  english:
    "The Reporter Foundation was founded with T.H. Tung's personal donation of NTD 5 million, and depends on donations to maintain operations hereafter.",
}

export const mediaIntro = {
  chinese:
    '《報導者》為「財團法人報導者文化基金會」所成立的非營利網路媒體，致力於公共領域深度調查報導。',
  english:
    'The Reporter is a non-profit media organization founded by The Reporter Foundation and relies on public donation. We focus on in-depth reportage and investigative journalism.',
}

export const rules = {
  chinese: [
    '「財團法人報導者文化基金會」 董事會通過下列工作倫理規範：',
    '1. 基金會董監事不得涉入與干預《 報導者》 任何新聞處理個案與新聞內容。',
    '2. 所有捐款者皆秉持「 三不原則」： 不擁有、 不干預、 不回收，《 報導者》 任何新聞處理皆不受捐款者影響。',
    '3. 新聞處理內容若涉及上述兩條文，由監察人監督內容是否維持獨立自主。',
    '4. 凡基金會專職人員，在外不得有任何支領固定酬勞之兼職；除經基金會同意，不得於商業媒體撰寫文稿。',
  ],
}
