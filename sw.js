...
self.addEventListener('fetch', function(e) {
  if (isPathWhitelisted(["^\\/(a|topics|tags|category|categories)\\/","^\\/$","^\\/photography"], e.request.url)) {
            ...
            .then(function(response) {
              ...
              .then(function(matchingClients) {
                if (matchingClients[0]) {
                  const redirectURL = new URL(e.request.url).origin + '/sw-fallback-page' + '?redirect=' + e.request.url
  ...
  } else if (e.request.url.indexOf('/sw-fallback-page') > -1) {
    console.log('[Service Worker] CacheFirst. Fetch fallback page', '/sw-fallback-page');
    return e.respondWith(
      caches.open(cacheName).then(function(cache) {
...
toolbox.router.get(/http:\/\/localhost(:8080)?\/v1\/posts/, toolbox.networkFirst, {"cache":{"name":"posts-cache","maxEntries":50}});
toolbox.router.get(/http:\/\/localhost(:8080)?\/v1\/topics/, toolbox.networkFirst, {"cache":{"name":"topics-cache","maxEntries":10}});
toolbox.router.get(/http:\/\/localhost(:8080)?\/v1\/index_page(_categories)?/, toolbox.networkFirst, {"cache":{"name":"index-page-cache"}});
