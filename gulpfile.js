...
gulp.task('build-packages', function (done) {
  async.auto({
    buildJs: function(callback) {
      ...
      process.env.WEBPACK_CONFIG = './webpack/pkg.config.js';
    ...
    buildBootstrap:[ 'buildJs', function(callback) {
      ...
      spawn('webpack', ['--verbose', '--colors', '--display-error-details', '--config', './webpack/bootstrap.config.js'], {stdio: 'inherit'})
        .on('close', callback)
        .on('error', function(err) {
          callback(err);
        })
    ...
    buildCss: ['buildBootstrap', function(callback) {
      ...
      async.auto({
        ...
        removeFolder: [ 'concatCss', function(cb) {
          spawn('rimraf', ['lib-css'], {stdio: 'inherit'})
            .on('close', cb)
            .on('error', function(err) {
              cb(err)
            })
    ...
    build: ['buildCss', function(callback) {
